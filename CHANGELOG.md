...
- 長語音段超過 history 保留範圍時，仍保存 VAD 交回的完整音訊，避免定稿切片截掉前段。
- 停止、暫停、切換輸入與重新連線前，先停止 callback，再排空已收音訊及重採樣尾音；
  寫檔失敗時保留待處理音訊供重試。
...
- 提供音訊覆蓋、完整預覽重播與定稿模型的隔離比較工具，記錄來源指紋、CPU 與記憶體。
- 增加長段、段落交界、停止期間 callback、VAD 故障及反覆寫檔失敗的音訊完整性驗證。

### 驗證界線

- 此為測試版，保留既有預設模型與執行緒數。尚無可信的 Windows 原生字幕同音源比較，
  也未完成歌曲人工聽校；不宣稱辨識準確率或畫面延遲已改善。
