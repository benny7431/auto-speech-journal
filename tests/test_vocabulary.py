...
def test_delete_term_keeps_other_terms_and_all_correction_locks(
    tmp_path: Path,
    storage: JournalStorage,
) -> None:
    vocabulary = VocabularyStore(storage)
...
def test_delete_term_handles_missing_and_empty_values(
    tmp_path: Path,
    storage: JournalStorage,
) -> None:
    vocabulary = VocabularyStore(storage)
...
def test_clear_vocabulary_keeps_correction_locks_and_is_safe_when_empty(
    tmp_path: Path,
    storage: JournalStorage,
) -> None:
    vocabulary = VocabularyStore(storage)
...
def test_hotwords_are_limited_by_count_total_chars_and_frequency(
    tmp_path: Path,
    storage: JournalStorage,
) -> None:
...
def test_deleting_one_hour_keeps_other_segments_shared_term_count(
    tmp_path: Path,
    storage: JournalStorage,
) -> None:
    vocabulary = VocabularyStore(storage)
